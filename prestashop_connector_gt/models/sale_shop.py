...
class SaleShop(models.Model):
			...
			if attr_val.get('associations').get('product_option_values').get('product_option_value'):
				...
				for attrs in attributes:
					...
					attrs_op_values = {
						...
						"name": self.get_value_data(self.get_value(product_options.get('name').get('language'))) + ' [' +  self.get_value_data(product_options.get('id')) + ']',
					}
	...
	def get_value(self, data):
		...
		if isinstance(data, list):
			data = data
			lang_id = self.env['prestashop.language'].search([('code','=','fr'), ('presta_instance_id','=', self.prestashop_instance_id.id)])
			if not lang_id:
				lang = self.env['prestashop.language'].search([])
	...
	def manageOrderLines(self, orderid, order_detail, prestashop):
		...
		for child in order_rows:
			...
			if more_order_details and more_order_details.get('order_detail'):
				...
				if vat_id and vat_id.get('id'):
					...
					if vat_rate and vat_rate.get('tax'):
						...
						if vat_rate:
							tax_odoo = self.env['account.tax'].sudo().search([('type_tax_use', '=', 'sale'),('amount','=', vat_rate)], limit = 1)
							if tax_odoo:
								tax_id = tax_odoo.id
								line.update({'price_unit': str(self.get_value_data(child.get('unit_price_tax_excl')))})
			...
			if self.get_value_data(child.get('product_attribute_id')) != '0':
				...
				if temp_ids:
					product_ids = product_obj.search([('product_tmpl_id', '=', temp_ids[0].id),('combination_id','=', self.get_value_data(child.get('product_attribute_id')))])
					if not product_ids:
						...
						if product_id.product_template_attribute_value_ids:
							for product_template_attribute_value_id in product_id.product_template_attribute_value_ids:
								...
								if product_template_attribute_value_id.name:
									...
									product_attribute_name = re.sub("[\[].*?[\]]", "", product_attribute_name).rstrip()
								...
								if value_list:
									...
									for value_list_attribute in value_list_attributes:
										value_list_attribute_name = re.sub("[\[].*?[\]]", "", value_list_attribute.name).rstrip()
										if product_attribute_name == value_list_attribute_name and product_id.product_tmpl_id == temp_ids[0]:
											count_ok += 1
					...
					if new_product_ids:
						...
						if not p_ids:
							pid = product_obj.create({'name': 'Removed Product', 'default_code': 'RMVPRD' + str(removed_product)})
						else:
							pid = p_ids[0]
		...
		if discoun and discoun > 0:
			...
			if tax_id:
				dline.update({'price_unit': (-1 * float(self.get_value_data(order_detail.get('total_discounts_tax_excl'))))})
				dline.update({'tax_id': [(4, tax_id)]})
			dline_ids = sale_order_line_obj.search([('product_id', '=', self.get_value_data(dline.get('product_id'))),('order_id', '=', orderid.id)])
			if not dline_ids:
		...
		if ship and ship > 0:
			...
			if vat_rate:
				...
				if vat_rate:
					tax_odoo = self.env['account.tax'].sudo().search([('type_tax_use', '=', 'sale'),('amount','=', vat_rate)], limit = 1)
					if tax_odoo:
						sline.update({'price_unit': float(self.get_value_data(order_detail.get('total_shipping_tax_excl')))})
	...
	def import_orders(self):
		...
		for shop in self:
			...
			order_data = prestashop.get('orders',options={'filter[invoice_date]': '[' + str_start_datetime + ',' + str_end_datetime + ']','date':'1'})
